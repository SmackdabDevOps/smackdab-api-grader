        ...
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          ['get', 'post', 'put', 'patch', 'delete'].forEach(method => {
            if (pathItem[method]?.responses?.['401']) {
              ...
              if (!response.$ref && !response.headers?.['WWW-Authenticate']) {
            ...
            if (pathItem[method]?.responses) {
              ['200', '201', '202', '204'].forEach(status => {
                if (pathItem[method].responses[status] && 
                    !pathItem[method].responses[status].headers?.['X-Request-ID']) {
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          if (pathItem.delete?.responses?.['204']) {
            const headers = pathItem.delete.responses['204'].headers;
          ...
          ['get', 'post', 'patch', 'put', 'delete'].forEach(method => {
            if (pathItem[method]?.responses) {
              ['200', '201', '202', '204', '206'].forEach(status => {
                if (pathItem[method].responses[status]) {
                  const headers = pathItem[method].responses[status].headers;
                  if (!headers?.['X-RateLimit-Limit'] || 
                      !headers?.['X-RateLimit-Remaining'] ||
                      !headers?.['X-RateLimit-Reset']) {
                    errors.push(`${method.toUpperCase()} ${path} ${status}`);
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          if (pathItem.get?.responses?.['200']) {
            if (!pathItem.get.responses['200'].headers?.ETag) {
              errors.push(path);
            }
          }
        });
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          ...
          if (pathItem.post?.responses?.['201']) {
            if (!pathItem.post.responses['201'].headers?.ETag) {
          ...
          if (pathItem.patch?.responses?.['200']) {
            if (!pathItem.patch.responses['200'].headers?.ETag) {
  ...
  },
  {
    ...
    check: (spec) => {
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          if (pathItem.get?.responses?.['200']) {
            const headers = pathItem.get.responses['200'].headers;
            if (!headers?.Vary) errors.push(`GET ${path} missing Vary`);
            if (!headers?.['Cache-Control']) errors.push(`GET ${path} missing Cache-Control`);
          }
        });
      }
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          if (pathItem.post?.responses?.['202']) {
            const headers = pathItem.post.responses['202'].headers;
            if (!headers?.Location) {
...
function main() {
  ...
  if (args.length === 0) {
    console.log('Usage: node comprehensive-grader-v2.cjs <openapi-spec.yaml>');
  ...
  try {
    ...
    const reportPath = filePath.replace('.yaml', '-v2-report.json').replace('.yml', '-v2-report.json');
