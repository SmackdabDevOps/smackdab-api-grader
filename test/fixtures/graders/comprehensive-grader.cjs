...
/**
 * Smackdab API Contract Grader v2.0 - Complete Enforcement
 * Based on MASTER_API_TEMPLATE_v3.yaml standards
 * 
 * This version implements ALL validation checks from the master template
 * with proper parameter inheritance and comprehensive gap coverage
 */
...
let glob; try { glob = require('glob'); } catch (_) { glob = null; }
...
/**
 * Stable sort helpers so results are deterministic
 */
const httpMethods = ['get','post','put','patch','delete','options','head'];
function sortedPaths(spec){
  return Object.keys(spec.paths || {}).sort((a,b)=>a.localeCompare(b));
}
function sortedMethods(pathItem){
  return httpMethods.filter(m=>pathItem[m]);
}
...
/** Case-insensitive header presence on a response headers object */
function hasHeaderCI(headers, name){
  return !!getHeader(headers, name);
}

/** Ensure response has no body (no content or empty object) */
...
function extractPathParamsFromTemplate(p){
  const out = [];
  const re = /\{([^}]+)\}/g; let m;
...
/** Component header existence (case-insensitive) */
function hasComponentHeader(components, name){
  if (!components?.headers) return false;
  return Object.keys(components.headers).some(k => k.toLowerCase() === name.toLowerCase());
}

/** Ensure parameter has required=true and correct location when requested */
function paramSatisfies(p, expected){
  if (!p) return false;
  if (expected?.in && p.in !== expected.in) return false;
  if (expected?.required === true && p.required !== true) return false;
  return true;
}

/** Hash an api spec to provide a stable api_id in reports */
function stableApiId(spec){
  try{
    const h = crypto.createHash('sha256');
    h.update(JSON.stringify(spec, Object.keys(spec).sort()));
    return h.digest('hex').slice(0,12);
  }catch(_){ return undefined; }
}

/** Identify collection vs. item path */
function isItemPath(p){
  return /\}\s*$/.test(p);
}

/** Simple plural heuristic and kebab-case check */
function isPluralKebab(segment){
  const kebab = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
  const plural = /s$/;
  return kebab.test(segment) && plural.test(segment);
}

/** Try to build an approximate jsonPath for findings */
function jp(parts){
...
/**
 * Check if operation has a specific parameter (by $ref or name) with stricter matching
 */
function hasParameter(pathItem, operation, paramNameOrRef, expected={}) {
  const params = resolveEffectiveParams(pathItem, operation);
  return params.some(p => {
    const match = p.$ref
      ? (p.$ref === paramNameOrRef || p.$ref.endsWith('/' + paramNameOrRef))
      : (p.name === paramNameOrRef);
    return match && paramSatisfies(p, expected);
  });
}
...
function findAllPlaceholders(spec) {
  ...
  while ((match = placeholderPattern.exec(jsonStr)) !== null) {
    ...
    if (['domain', 'Domain', 'Resource', 'resource', 'Resources', 'resources', 
         'API Name', 'Specific Function', 'id', 'job_id', 'key'].includes(placeholder)) {
...
const CHECKPOINTS = [
  ...
  // ========== SERVERS (2 points) ==========
  {
    ...
    check: (spec) => {
      ...
      const hasProduction = urls.includes('https://api.smackdab.com');
      const hasStaging = urls.includes('https://staging-api.smackdab.com');
        ...
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          ...
          ['post', 'patch', 'put', 'delete'].forEach(method => {
            if (pathItem[method] && pathItem[method].security) {
              ...
              if (!hasWriteScope && pathItem[method].security.some(sec => sec.OAuth2)) {
  ...
  },
  {
    ...
    check: (spec) => {
      ...
      if (!hasHeaderCI(unauthorized.headers, 'WWW-Authenticate')) {
        return { passed: false, message: '401 response missing WWW-Authenticate header' };
      }
      
      // Also check that 401 responses in operations reference this component
      const errors = [];
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          ['get', 'post', 'put', 'patch', 'delete'].forEach(method => {
            if (pathItem[method]?.responses?.['401']) {
              ...
              if (!response.$ref && !hasHeaderCI(response.headers, 'WWW-Authenticate')) {
                errors.push(`${method.toUpperCase()} ${path} 401 response missing WWW-Authenticate`);
              }
            }
          });
        });
            ...
            if (pathItem[method]?.responses) {
              ['200', '201', '202', '204'].forEach(status => {
                if (pathItem[method].responses[status] && 
                    !hasHeaderCI(pathItem[method].responses[status].headers, 'X-Request-ID')) {
  ...
  },
  {
    id: 'MT-CONSISTENCY',
    category: 'tenancy',
    description: 'X-Consistency parameter for read operations',
    weight: 1,
    autoFail: false,
    check: (spec) => {
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          if (pathItem.delete?.responses?.['204']) {
            const h = pathItem.delete.responses['204'].headers || {};
            if (!hasHeaderCI(h,'X-RateLimit-Limit') || !hasHeaderCI(h,'X-RateLimit-Remaining') || !hasHeaderCI(h,'X-RateLimit-Reset')) {
              errors.push(path);
            }
          }
        });
  ...
  {
    id: 'HTTP-DELETE-204-NOBODY',
    category: 'http',
    description: 'DELETE 204 responses must not include a body',
    weight: 1,
    autoFail: false,
    check: (spec) => {
      const offenders = [];
      if (spec.paths){
        Object.entries(spec.paths).forEach(([p,pi])=>{
          const r = pi.delete?.responses?.['204'];
          if (r && !hasNoBody(r)) offenders.push(p);
        });
      }
      return offenders.length===0 ? {passed:true} : {passed:false, message:`DELETE 204 must be bodyless: ${offenders.slice(0,5).join(', ')}`};
    }
  },
  {
    id: 'HTTP-201-LOCATION',
    category: 'http',
    description: 'POST 201 responses include Location header',
    weight: 1,
    autoFail: false,
    check: (spec) => {
      const missing = [];
      if (spec.paths){
        Object.entries(spec.paths).forEach(([p,pi])=>{
          const r = pi.post?.responses?.['201'];
          if (r){
            const h = r.headers || {};
            if (!hasHeaderCI(h,'Location')) missing.push(p);
          }
        });
      }
      return missing.length===0 ? {passed:true} : {passed:false, message:`POST 201 missing Location: ${missing.slice(0,5).join(', ')}`};
    }
  },
  {
    id: 'HTTP-429-RETRY-AFTER',
    category: 'http',
    description: '429 responses define and use Retry-After header',
    weight: 1,
    autoFail: false,
    check: (spec) => {
      const comp = spec.components?.responses?.TooManyRequests;
      if (comp && !hasHeaderCI(comp.headers,'Retry-After')){
        return {passed:false, message:'TooManyRequests component missing Retry-After header'};
      }
      const offenders = [];
      if (spec.paths){
        Object.entries(spec.paths).forEach(([p,pi])=>{
          sortedMethods(pi).forEach(m=>{
            const r = pi[m]?.responses?.['429'];
            if (r && !hasHeaderCI(r.headers,'Retry-After')) offenders.push(`${m.toUpperCase()} ${p}`);
          });
        });
      }
      return offenders.length===0 ? {passed:true} : {passed:false, message:`429 missing Retry-After on: ${offenders.slice(0,5).join(', ')}`};
    }
  },
          ...
          ['get', 'post', 'patch', 'put', 'delete'].forEach(method => {
            if (pathItem[method]?.responses) {
              ['200', '201', '202', '204', '206'].forEach(status => {
                if (pathItem[method].responses[status]) {
                  const h = pathItem[method].responses[status].headers || {};
                  if (!hasHeaderCI(h,'X-RateLimit-Limit') || !hasHeaderCI(h,'X-RateLimit-Remaining') || !hasHeaderCI(h,'X-RateLimit-Reset')) {
                    errors.push(`${method.toUpperCase()} ${path} ${status}`);
                  }
                }
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          if (pathItem.get?.responses?.['200']) {
            const h = pathItem.get.responses['200'].headers || {};
            if (!hasHeaderCI(h,'ETag')) errors.push(path);
          }
        });
      }
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          ...
          if (pathItem.post?.responses?.['201']) {
            const h1 = pathItem.post.responses['201'].headers || {};
            if (!hasHeaderCI(h1,'ETag')) warnings.push(`POST ${path} 201`);
          }
          
          // Check PATCH 200
          if (pathItem.patch?.responses?.['200']) {
            const h2 = pathItem.patch.responses['200'].headers || {};
            if (!hasHeaderCI(h2,'ETag')) warnings.push(`PATCH ${path} 200`);
          }
        });
      }
  ...
  },
  {
    ...
    category: 'caching',
    description: 'Vary and Cache-Control headers on GET 200',
    weight: 1,
    autoFail: false,
    check: (spec) => {
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          if (pathItem.get?.responses?.['200']) {
            const h = pathItem.get.responses['200'].headers || {};
            if (!hasHeaderCI(h,'Vary')) errors.push(`GET ${path} missing Vary`);
            if (!hasHeaderCI(h,'Cache-Control')) errors.push(`GET ${path} missing Cache-Control`);
          }
        });
      }
  ...
  // ========== PAGINATION (6 points) ==========
  {
    ...
    autoFail: true,
    check: (spec) => {
      // Check for forbidden pagination parameters
      ...
      if (foundForbidden.length > 0) {
        return { passed: false, message: `Found forbidden pagination params: ${foundForbidden.join(', ')}` };
      }
  ...
  },
  {
    ...
    check: (spec) => {
      ...
      return filters.style === 'deepObject' ? 
        { passed: true } : 
        { passed: false, message: 'Filter parameter must use deepObject style' };
    }
  },
  {
    id: 'PAG-FIELDS',
      ...
      if (spec.paths) {
        Object.entries(spec.paths).forEach(([path, pathItem]) => {
          if (pathItem.post?.responses?.['202']) {
            const h = pathItem.post.responses['202'].headers || {};
            if (!hasHeaderCI(h,'Location')) { errors.push(`POST ${path} 202 missing Location`); }
            if (!hasHeaderCI(h,'Retry-After')) { errors.push(`POST ${path} 202 missing Retry-After`); }
            if (!hasHeaderCI(h,'X-RateLimit-Limit') || !hasHeaderCI(h,'X-RateLimit-Remaining') || !hasHeaderCI(h,'X-RateLimit-Reset')) {
  ...
  // ========== PATH RULES (3 points) ==========
  {
    id: 'PATH-V2-PREFIX',
    category: 'paths',
    description: 'All paths start with /api/v2/',
    weight: 2,
    autoFail: true,
    check: (spec) => {
      const bad = sortedPaths(spec).filter(p => !p.startsWith('/api/v2/'));
      return bad.length === 0 ? {passed:true} : {passed:false, message:`Non-compliant path prefix: ${bad.slice(0,5).join(', ')}`};
    }
  },
  {
    id: 'PATH-COLLECTION-SEGMENTS',
    category: 'paths',
    description: 'Collection segments are plural kebab-case',
    weight: 1,
    autoFail: false,
    check: (spec) => {
      const issues = [];
      sortedPaths(spec).forEach(p => {
        const parts = p.split('/').filter(Boolean);
        for (let i=0;i<parts.length;i++){
          const seg = parts[i];
          if (seg.startsWith('{')) continue; // param
          // treat last segment specially: if not a param and followed by no param, could be item path; skip
          const isLast = i === parts.length-1;
          const nextIsParam = !isLast && parts[i+1] && parts[i+1].startsWith('{');
          const looksCollection = !isLast || nextIsParam; // collection when followed by a param id or not last
          if (looksCollection && !isPluralKebab(seg)){
            issues.push(p);
            break;
          }
        }
      });
      return issues.length === 0 ? {passed:true} : {passed:false, message:`Non-plural or non-kebab segments: ${issues.slice(0,5).join(', ')}`};
    }
  },
  // ========== PARAMETERS (2 points) ==========
  {
    id: 'PARAMS-PATH-REQUIRED',
    category: 'params',
    description: 'All path template params are defined with in:path and required:true',
    weight: 2,
    autoFail: true,
    check: (spec) => {
      const issues = [];
      sortedPaths(spec).forEach(p => {
        const names = extractPathParamsFromTemplate(p);
        if (names.length===0) return;
        const pi = spec.paths[p];
        const present = new Map();
        httpMethods.forEach(m=>{
          if (!pi[m]) return;
          const eff = resolveEffectiveParams(pi, pi[m]);
          eff.forEach(pp => {
            const name = pp.$ref ? pp.$ref.split('/').pop() : pp.name;
            if (name) present.set(name, pp);
          });
        });
        names.forEach(n => {
          const candidate = present.get(n) || present.get(n.replace(/\W/g,''));
          if (!candidate || candidate.in !== 'path' || candidate.required !== true){
            issues.push(`${p} {${n}}`);
          }
        });
      });
      return issues.length===0 ? {passed:true} : {passed:false, message:`Path params must be in:path & required: ${issues.slice(0,5).join(', ')}`};
    }
  },
  {
  ...
  },
  {
    ...
    category: 'async',
    description: 'Accept-Version parameter defined',
    weight: 1,
    autoFail: false,
    check: (spec) => {
      ...
      // Check pattern
      if (!acceptVersion.schema?.pattern?.includes('v[0-9]')) {
  ...
  },
  {
    ...
    check: (spec) => {
      ...
      const hasV2Path = paths.some(p => p.includes('/api/v2/'));
      
      return hasV2Path ? 
        { passed: true } : 
        { passed: false, message: 'API paths must use /api/v2/{domain}/{resources} format' };
    }
  },
...
function gradeAPI(spec) {
  ...
  const totalPossible = CHECKPOINTS.reduce((sum, c) => sum + c.weight, 0);
  let normalized = Math.round((totalScore / Math.max(1,totalPossible)) * 100);
  if (autoFailReasons.length > 0) normalized = Math.min(normalized, 59);
  return {
    api_id: stableApiId(spec),
    rawScore: totalScore,
    score: normalized,
    letterGrade: getLetterGrade(normalized),
    findings,
    autoFailReasons,
    autoFailIds,
    totalPossible
...
function parseArgs(argv){
  const out = { files: [], pattern: null, strict: false, minScore: null };
  for (let i=0;i<argv.length;i++){
    const a = argv[i];
    if (a === '--strict') out.strict = true;
    else if (a.startsWith('--min-score=')) out.minScore = parseInt(a.split('=')[1],10);
    else if (a.startsWith('--pattern=')) out.pattern = a.split('=')[1];
    else if (!a.startsWith('--')) out.files.push(a);
  }
  return out;
}

async function gradeOne(filePath){
  const fileContents = fs.readFileSync(filePath, 'utf8');
  const spec = yaml.load(fileContents);
  const result = gradeAPI(spec);
  // Save detailed report per file
  const reportPath = filePath.replace(/\.ya?ml$/i, '-v2-report.json');
  fs.writeFileSync(reportPath, JSON.stringify({ file: filePath, ...result }, null, 2));
  return { file: filePath, ...result };
}

async function run(){
  const argv = parseArgs(process.argv.slice(2));
  if (!argv.pattern && argv.files.length === 0){
    console.log('Usage: node comprehensive-grader-v2.cjs [--strict] [--min-score=NN] [--pattern="glob"] <openapi-spec.yaml>');
    console.log('\nSmackdab API Contract Grader v2.0');
    console.log('Complete enforcement of MASTER_API_TEMPLATE_v3.yaml standards');
    console.log(`\nTotal checkpoints: ${CHECKPOINTS.length}`);
    console.log(`Total possible weight: ${CHECKPOINTS.reduce((sum, c) => sum + c.weight, 0)} (normalized to 100)`);
    process.exit(1);
  }
  let targets = [...argv.files];
  if (argv.pattern){
    if (!glob) { console.error('glob module not installed. Run: npm i glob'); process.exit(2); }
    targets = [...targets, ...glob.sync(argv.pattern, { nodir: true })];
  }
  targets = Array.from(new Set(targets));
  if (targets.length === 0){ console.error('No files matched.'); process.exit(1); }

  let anyAutoFail = false, anyBelow = false;
  const results = [];
  for (const f of targets){
    console.log(`${colors.blue}📊 Grading: ${f}${colors.reset}`);
    console.log('='.repeat(60));
    try{
      const r = await gradeOne(f);
      results.push(r);
      console.log(`\n${colors.bright}🎯 Score: ${r.score}/100 (${r.letterGrade})${colors.reset}`);
      if (r.autoFailReasons.length > 0){
        anyAutoFail = true;
        console.log(`\n${colors.red}❌ AUTO-FAIL TRIGGERED (${r.autoFailIds.length} violations):${colors.reset}`);
        r.autoFailReasons.forEach(reason => console.log(`  - ${reason}`));
      }
      // Group findings by category (short console view)
      const categories = {};
      CHECKPOINTS.forEach(cp => {
        if (!categories[cp.category]) categories[cp.category] = { total: 0, passed: 0 };
        categories[cp.category].total += cp.weight;
      });
      r.findings.forEach(fnd => { if (fnd.passed) categories[fnd.category].passed += fnd.weight; });
      ...
      Object.entries(categories).forEach(([cat, d]) => {
        ...
        console.log(`  ${cat.toUpperCase()}: ${pct}%`);
      });
      if (argv.minScore != null && r.score < argv.minScore) anyBelow = true;
    }catch(e){
      console.error(`${colors.red}Error grading ${f}: ${e.message}${colors.reset}`);
  ...
  const summary = {
    ...
    min_score: argv.minScore,
    strict: argv.strict,
    results: results.map(({file, api_id, score, letterGrade, autoFailIds, autoFailReasons})=>({file, api_id, score, letterGrade, autoFail: autoFailIds?.length>0, autoFailCount: autoFailIds?.length||0, autoFailReasons}))
  }; 
  fs.writeFileSync('grader-summary.json', JSON.stringify(summary, null, 2));
  console.log(`\n${colors.green}📄 Summary saved to grader-summary.json${colors.reset}`);
  if (argv.strict || argv.minScore != null){
    if (anyAutoFail || anyBelow){
      console.error(`${colors.red}Build failed: ${anyAutoFail?'auto-fail detected':''}${anyAutoFail&&anyBelow?' and ':''}${anyBelow?`score below ${argv.minScore}`:''}${colors.reset}`);
      process.exit(3);
    }
  }
}
